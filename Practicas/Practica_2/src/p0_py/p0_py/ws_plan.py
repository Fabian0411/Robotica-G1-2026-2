...
class ScaraControl(Node):
    ...
    def cbck_scara_control(self):
        ...
        for i in range(1, 11):
            ...
            x_t = x_i + (10*pow(t_sim,3)-15*pow(t_sim,4)+6*pow(t_sim,5)
                        )*x_j -x_i
            y_t = y_i + (10*pow(t_sim,3)-15*pow(t_sim,4)+6*pow(t_sim,5)
                        )*y_j -y_i
            theta_t = theta_i + (10*pow(t_sim,3)-15*pow(t_sim,4)+6*pow(t_sim,5)
                        )*theta_j -theta_i
